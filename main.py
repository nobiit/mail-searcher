...
class Tasks:
    ...
    def schedule(self, action, key, value):
        ...
        if action not in self._jobs:
            self._jobs[action] = {
                '__type__': 'pairs' if key else 'values'
            }
        d = self._jobs[action]
        if key:
...
class Email:
    ...
    def date(self):
        try:
            ...
            return parsedate_to_datetime(self.decode_header(self._msg['Date']))
...
def get_labels(item):
    ...
    if list(filter(lambda x: x not in [Email.FLAG_SEEN, Email.FLAG_ANSWERED, Email.FLAG_STARRED, Email.FLAG_FLAGGED, Email.FLAG_DELETED], item.flags)):
        print('OK')

    # if item.address.capture([r'.+@(?P<name>[\w-]+)\.groups\.io', r'(?P<name>[\w-]+)@(groups\.io|googlegroups\.com|nongnu\.org|cml\.news|w3\.org|lists\.(strace\.io))'], matches, many=True):
    #     def get_name(v):
    #         items = {
    #             'Airspy': ['airspy'],
    #             'Beta': ['beta'],
    ...
    if item.address.fnmatch('*@itviec.com'):
        labels.add('Other/HR')
        if match(r'(.+ jobs? for you|.+, looking for a .+ job?)', item.subject):
            if item.days > 3:
                labels.add(Email.LABEL_TRASH)
    ...
    with Mail(user=environ.get('IMAP_USER'), passwd=environ.get('IMAP_PASSWD')) as m:
        ...
        for item in messages:
            ...
            try:
                ...
                invalid_labels = list(filter(lambda x: x[0] != '\\' and x not in labels, item.labels))
